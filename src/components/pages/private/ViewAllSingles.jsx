...
const ViewAllSingles = () => {
  ...
  if (!singleData) {
    return (
      <>
        <h1 className='text-2xl pb-2 font-bold underline underline-offset-[3px] decoration-niceOrange-400 decoration-2'>My singles</h1>
  ...
  return (
    <>
      <h1 className='text-2xl pb-2 font-bold underline underline-offset-[3px] decoration-niceOrange-400 decoration-2'>My single</h1>

      <ul className='grid grid-cols-[repeat(_auto-fill,minmax(150px,1fr)_)] grid-flow-dense mini:gap-[15px] gap-[10px] w-full'>
        {singleData.map((single) => (
          <li key={single.song_id}>
            <SingleCard
              name={single.name}
              year={new Date(single.creation_date).getFullYear()}
              id={single.song_id}
