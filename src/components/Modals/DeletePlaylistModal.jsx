...
export default function DeletePlaylistModal ({ playlistId, playlistName }) {
  ...
  const { deleteObjectFromPlaylist } = useContext(playlistContext)
  ...
  const handleDeletePlaylists = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/private/playlist/${playlistId}`, options)
      .then(response => response.json())
      .then(async res => {
        if (!res.error) {
          deleteObjectFromPlaylist(parseInt(playlistId))
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose} backdrop='blur' scrollBehavior={'inside'} placement={'center'}>
        <ModalContent>
          ...
          <ModalBody>
            <p>
              Are you certain you wish to delete playlist
              &quot;
          ...
          <ModalFooter>
            <Button isLoading={isLoading} color="danger" variant='flat' onClick={handleDeletePlaylists}>
              Delete
            </Button>
